...
### 6.4 视觉辨识度护栏（frontend-design 校验）

跑 `frontend-design` 复核三朝，避免落入 AI 默认三套（暖奶油+高对比衬线+赭红 / 近黑+荧光色 / 大报式发丝线密栏）：

- **类型即朝代签名**（强化 §3）：display 字体本身是「迥然不同」的一条**独立轴**，不只靠配色——明＝印刷体宋（官刻奏章的方正）· 唐＝楷书（盛唐碑刻/写经的开张）· 宋＝瘦金体（徽宗御笔的清癯）。三朝并排，仅看标题字形即可辨朝代。
- **明的反套路护栏**：明（宣纸 `#F5F0E8` + 宋体 + 朱砂红）字面接近 AI 默认套路一。**靠题材材质拉开**：朱砂红是「印泥/朱批朱」这一具体的墨，以**真实笔触（DrawSVG 朱批）+ 朱印钤记**出现，**绝非平涂赭红色块**；signature＝制衡天平。宣纸暖白是奏章题材的材质真值（ADR-0001 锁定），非自由轴上的随手默认。
- **不滑向大报式**：A4 的细墨发丝边 + 编辑克制若无质感支撑会沦为「大报默认」。辨识度由**宣纸层叠材质 + 水墨晕开动效 + 招牌场景**承载；boldness 只花在招牌，chrome 保持安静。
- **结构编码意义**：分区顺序须承载制度逻辑（如中枢→监察→军事的真实层级），是真序列才可编号；不为装饰而编号。
